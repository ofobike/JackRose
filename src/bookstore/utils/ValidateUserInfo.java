...
public class ValidateUserInfo {
    ...
    private static  final  String REGEX_PHONE = "^(13[0-9]|14[5|7]|15[0|1|2|3|5|6|7|8|9]|18[0|1|2|3|5|6|7|8|9])\\d{8}$";
    /**
     * 对所有信息进行验证
     *
     * @return 合法返回null，否则返回错误信息
     */
    public static String validate(String username,String password,String phone,String address,String email) {
        StringBuilder sb = new StringBuilder();
        String temp;

        if ((temp = validatePassword(password)) != null) {
            sb.append(temp + "；");
        }
        if ((temp = validateName(username)) != null) {
            sb.append(temp + "；");
        }
        if ((temp = validateEmail(email)) != null) {
            sb.append(temp + "；");
        }
        if ((temp = validatePhone(phone)) != null) {
            sb.append(temp + "；");
        }
        if ((temp = validateAddress(address)) != null) {
            sb.append(temp + "；");
        }
    ...
    private static String validateAddress(String address) {
        if (address == null || "".equals(address) || address.length() > 255) {
            return "地址为1到255个字符";
        }
        return null;
    }
